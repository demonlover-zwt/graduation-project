...
fig,ax=plt.subplots(figsize=(9, 6))   ###(宽，高)
plt.xlim(np.min(cable_x[pos_ini-1:(pos_fin-1)+1])-0.5*y_span, np.max(cable_x[pos_ini-1:(pos_fin-1)+1])+0.5*y_span)
plt.ylim(np.min(cable_y[pos_ini-1:(pos_fin-1)+1])-0.5*x_span, np.max(cable_y[pos_ini-1:(pos_fin-1)+1])+0.5*x_span) 
plt.plot(cable_x[know_ini-1:(know_fin-1)+1], cable_y[know_ini-1:(know_fin-1)+1], color="k", linestyle="-", linewidth=2.5, zorder=2)
plt.plot(cable_x[(pos_ini-1)-1:(pos_fin-1)+1], cable_y[(pos_ini-1)-1:(pos_fin-1)+1], color="#FF6A6A", linestyle="-", linewidth=2, zorder=1)   ### 参考位置
plt.scatter(cable_x[pos_ini-1:(pos_fin-1)+1], cable_y[pos_ini-1:(pos_fin-1)+1], color='#B22222', s=25, marker='o', zorder=3)   ### 参考位置
plt.scatter(channel_ind[0, :], channel_ind[1, :], color='b', s=25, marker='o', zorder=4)   ### 估计位置
plt.title("position angle: "+str(theta)+"°", fontsize=16)
