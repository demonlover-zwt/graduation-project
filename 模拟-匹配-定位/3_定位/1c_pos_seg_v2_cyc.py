...
gauss_kruger = pyproj.CRS('+proj=tmerc +lat_0=0 +lon_0=123 +k=1 +x_0=500000 +y_0=0 +datum=WGS84')
...
### 控制定位段
num_know=150   ### 已知段通道数量
num_pos=5  ### 每次定位的通道数
for num_know in [5, 10, 20, 50, 100, 150]:
    for num_pos in [5, 10, 20]:

        # know_ini=2192
        know_fin=2341
        know_ini=know_fin-num_know+1
        pos_ini=2342
        pos_fin=pos_ini+num_pos-1
        ...
        channel_frag=str(num_know)+"-"+str(num_pos)
        ...
        fig, axes = plt.subplots(2, 3, figsize=(20, 14))   # 宽度 20 英寸，高度 14 英寸
        ax = axes[0, 0]
        ...
        ax.set_xlim(k[k_start], k[k_end])
        ax.set_ylim(freq_positive[f_start], freq_positive[f_end])
        ax.set_xlabel('Wavenumber (m$^-$$^1$)', fontsize=16)
        ...
        # plt.savefig("./"+ship_name+"/"+channel_frag+"/f1_mea_fk.png", dpi=600, bbox_inches='tight')
        # plt.close()
        ...
        for theta in pos_ang:
            ...
            channel_ind=new_channel(cable_x[(pos_ini-1)-1], cable_y[(pos_ini-1)-1], cable_x[(pos_ini-1)-2], cable_y[(pos_ini-1)-2], theta, dx, num_pos)
            ...
            if theta == pos_ang[0]:
                ### 画fk谱
                # fig, axes = plt.subplots(2, 3, figsize=(16, 14))   # 宽度 16 英寸，高度 14 英寸
                ax = axes[1, 0]
                cax=ax.imshow(fk_plot,aspect='auto', extent=[k[k_start], k[k_end], freq_positive[f_start], freq_positive[f_end]])
                cax.set_clim(plot_min, plot_max)
                cbar = fig.colorbar(cax, ax=ax, fraction=0.025, pad=0.04)
                ...
                ax.set_xlabel('Wavenumber (m$^-$$^1$)', fontsize=16)
                ax.set_ylabel('Frequency (Hz)', fontsize=16)
                ax.set_title("angle: "+str(theta)+"°", fontsize=16)
                ax.tick_params(labelsize=14)
                ...
                # plt.savefig("./"+ship_name+"/"+channel_frag+"/f2_sim_fk"+str(theta)+".png", dpi=600, bbox_inches='tight')
                # plt.close()
        ...
        # fig, axes = plt.subplots(2, 3, figsize=(16, 14))   # 宽度 16 英寸，高度 14 英寸
        ax = axes[0, 2]
        ax.plot(pos_ang, np.abs(mismatch), linestyle='-', color='r', linewidth=2, label='combine deviation', alpha=0.5)
        ax.plot(pos_ang, np.abs(mismatch_f), linestyle='--', color='b', linewidth=2, label='frequency deviation', alpha=0.5)
        ax.plot(pos_ang, np.abs(mismatch_k), linestyle='-.', color='g', linewidth=1, label='wavenumber deviation', alpha=1)
        ax.set_xlim(pos_ang[0], pos_ang[-1])
        ax.set_ylim(0, 1.1*np.nanmax(mismatch))
        ax.set_xlabel("Angle [°]", fontsize=16)
        ax.set_ylabel("Deviation", fontsize=16)
        ax.tick_params(labelsize=14)
        ax.legend()
        # plt.savefig("./"+ship_name+"/"+channel_frag+"/f3_deviation.png", dpi=600, bbox_inches='tight')
        # plt.close()

        ### 画定位结果
        x_span=np.max(cable_x[pos_ini-1:(pos_fin-1)+1])-np.min(cable_x[pos_ini-1:(pos_fin-1)+1])
        y_span=np.max(cable_y[pos_ini-1:(pos_fin-1)+1])-np.min(cable_y[pos_ini-1:(pos_fin-1)+1])

        theta=pos_ang[np.argmin(mismatch)]
        channel_ind=new_channel(cable_x[(pos_ini-1)-1], cable_y[(pos_ini-1)-1], cable_x[(pos_ini-1)-2], cable_y[(pos_ini-1)-2], theta, dx, num_pos)
        ...
        # fig, axes = plt.subplots(2, 3, figsize=(16, 14))   # 宽度 16 英寸，高度 14 英寸
        ax = axes[1, 2]
        ax.set_xlim(np.min(cable_x[pos_ini-1:(pos_fin-1)+1])-0.5*y_span, np.max(cable_x[pos_ini-1:(pos_fin-1)+1])+0.5*y_span)
        ax.set_ylim(np.min(cable_y[pos_ini-1:(pos_fin-1)+1])-0.5*x_span, np.max(cable_y[pos_ini-1:(pos_fin-1)+1])+0.5*x_span) 
        ax.plot(cable_x[know_ini-1:(know_fin-1)+1], cable_y[know_ini-1:(know_fin-1)+1], color="k", linestyle="-", linewidth=2.5, zorder=2)
        ax.plot(cable_x[(pos_ini-1)-1:(pos_fin-1)+1], cable_y[(pos_ini-1)-1:(pos_fin-1)+1], color="#FF6A6A", linestyle="-", linewidth=2, zorder=1)   ### 参考位置
        ax.scatter(cable_x[pos_ini-1:(pos_fin-1)+1], cable_y[pos_ini-1:(pos_fin-1)+1], color='#B22222', s=25, marker='o', zorder=3)   ### 参考位置
        ax.scatter(channel_ind[0, :], channel_ind[1, :], color='b', s=25, marker='o', zorder=4)   ### 估计位置
        ax.set_title("position angle: "+str(theta)+"°", fontsize=16)
        ...
        # 生成刻度位置（在高斯克吕格坐标系中）
        n_ticks = 6
        x_ticks_gk = np.linspace(x_min, x_max, n_ticks)
        y_ticks_gk = np.linspace(y_min, y_max, n_ticks)
        # 批量转换x轴刻度标签（经度）
        # 对于x轴刻度，使用y轴中点作为纬度参考
        y_mid = (y_min + y_max) / 2
        x_for_lon = np.column_stack([x_ticks_gk, np.full_like(x_ticks_gk, y_mid)])
        lon_ticks, _ = transformer.transform(x_for_lon[:, 0], x_for_lon[:, 1])
        # 批量转换y轴刻度标签（纬度）
        # 对于y轴刻度，使用x轴中点作为经度参考
        x_mid = (x_min + x_max) / 2
        y_for_lat = np.column_stack([np.full_like(y_ticks_gk, x_mid), y_ticks_gk])
        _, lat_ticks = transformer.transform(y_for_lat[:, 0], y_for_lat[:, 1])
        # 格式化刻度标签
        def format_lon(lon):
            return f'{lon:.2f}°E' if lon >= 0 else f'{abs(lon):.2f}°W'
        def format_lat(lat):
            return f'{lat:.2f}°N' if lat >= 0 else f'{abs(lat):.2f}°S'
        lon_labels = [format_lon(lon) for lon in lon_ticks]
        lat_labels = [format_lat(lat) for lat in lat_ticks]
        # 设置刻度位置和标签
        ax.set_xticks(x_ticks_gk)
        ax.set_xticklabels(lon_labels, rotation=15)
        ax.set_yticks(y_ticks_gk)
        ax.set_yticklabels(lat_labels, rotation=15)
        # 设置坐标轴标签
        ...
        # plt.savefig("./"+ship_name+"/"+channel_frag+"/f4_position.png", dpi=600, bbox_inches='tight')
        plt.savefig("./"+ship_name+"/"+channel_frag+".png", dpi=600, bbox_inches='tight')
        plt.close()